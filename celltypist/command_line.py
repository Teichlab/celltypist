...
@click.option("--hierarchical", is_flag=True, default=False, help="Use hierarchical cell type annotation (requires a hierarchical model).")
@click.option("-m", "--model", default=None, help="Model used for predictions. If not provided, default to the `Immune_All_Low.pkl` model for flat annotation and the `Human_Tissue_Immune_LCPN.pkl` model for hierarchical annotation.", type=str)
...
@click.option("-mo", "--mode", default="best_match", help="For flat annotation, choose the cell type with the largest score/probability as the final prediction (`best_match`), or enable a multi-label classification (`prob_match`), which assigns 0 (i.e., unassigned), 1, or >=2 cell type labels to each query cell. Ignored if `--hierarchical` is enabled.", type=click.Choice(['best_match','prob_match']), show_default=True)
@click.option("-ps", "--p-thres", default=0.5, help="For flat annotation, the probability threshold for multi-label classification. Ignored if `--mode` is `best_match`. Ignored if `--hierarchical` is enabled.", type=float, show_default=True)
@click.option("--no-compute-conf", is_flag=True, default=False, help="For hierarchical annotation, do not compute confidence scores (i.e., prediction probabilities) for each query cell at each hierarchical level. Ignored if `--hierarchical` is not enabled.")
@click.option("-ls", "--label-source", default="predicted_labels", help="For hierarchical annotation, the attribute from which to retrieve cell type labels for confidence scoring. Must be one of `predicted_labels` or `majority_voting`. Ignored if `--hierarchical` is not enabled.", type=click.Choice(['predicted_labels','majority_voting']), show_default=True)
...
@click.option("--plot-results", is_flag=True, default=False, help="For flat annotation, plot the prediction results as multiple figures as well. Ignored if `--hierarchical` is enabled.")
