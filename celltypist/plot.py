...
def treeviz(tree: Tree,
    ...
    for name, (x, y, is_leaf) in oriented_coords.items():
        if is_leaf:
            ax.plot(x, y, marker = leaf_shape, ms = node_size_map_processed[name], color = node_color_map_processed[name], ls = 'None', **leaf_dict)
            if show_leaf_label:
            ...
            ax.plot(x, y, marker = node_shape, ms = node_size_map_processed[name], color = node_color_map_processed[name], ls = 'None', **node_dict)
            if show_node_label:
