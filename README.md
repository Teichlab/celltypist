...
+ <details>
  ...
  |:---:        |:---:                                                                                                              |:---:                                                                                                                                                                                                                              |
  |tree         |A [Tree](https://celltypist.readthedocs.io/en/latest/celltypist.tree.Tree.html) object, or path to a tree JSON file|This argument is ignored when resuming a previous run with `save_strategy='checkpointed'` and `resume=True`, as an existing tree will be automatically retrieved from previous run                                                 |
  ...
  |mode         |The training mode, either `'LCPN'` (default) or `'LCL'`                                                            |For LCPN, the maximum number of local classifiers trained equals the number of internal nodes in the tree. For LCL, the maximum number of models trained corresponds to the tree depth minus one                                   |
  ...
  |out_dir      |Path to the directory where all local classifiers and the updated tree are written during training                 |An empty `out_dir` is required (or will be created) when starting a new run with `save_strategy='checkpointed'` and `resume=False`. If not provided, it defaults to `{tree.handle}_{mode}` in the current working directory        |
