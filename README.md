...
+ <details>
  ...
  sc.pp.log1p(adata)
  ```
  If you already have a list of highly variable genes for this dataset, subset `adata` to those genes using `adata = adata[:, HVGs].copy()`. Alternatively, identify highly variable genes de novo and subset `adata` accordingly. For example:
  ```python
...
+ <details>
  ...
  See `1.6.` for how the majority voting classifier works.
  ```bash
  ...
  Users can also provide their own over-clustering result to the `--over-clustering` option. This option can be specified in several ways:
     ...
     3) if none of the above is provided, will use a heuristic over-clustering approach, noted above.
  ```bash
  celltypist --indata /path/to/input/file --hierarchical --model Human_Tissue_Immune_LCPN.pkl --outdir /path/to/outdir --majority-voting --over-clustering /path/to/over_clustering/file
  ```
  There is also a `--min-prop` option (defaults to 0) which controls the minimum proportion of cells from the dominant cell type required to name a given subcluster by this cell type. Subcluster that fails to pass this proportion threshold will be assigned `Heterogeneous`.  
...
+ <details>
  ...
  |[add_children](https://celltypist.readthedocs.io/en/latest/celltypist.tree.Tree.html#celltypist.tree.Tree.add_children)        |Add one or more nodes under a given parent node in the tree                    |`tree.add_children(new_node1, new_node2, parent = 'CD4+ T cell')`                                       |The tree is modified and the list of added nodes is returned. The insertion index for the new node(s) can be specified via `pos`|
  ...
  |[move_node](https://celltypist.readthedocs.io/en/latest/celltypist.tree.Tree.html#celltypist.tree.Tree.move_node)              |Move a node (and its descendants) from its current parent to a new parent      |`tree.move_node('Naive CD4+ T cell', to = 'CD8+ T cell')`                                               |The tree is modified and the moved node is returned. The destination index for the new node can be specified via `pos`          |
